  ...
  return (
    <section className="container py-5">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Main slide */}
        <div className={`lg:col-span-2 bg-gradient-to-br ${slide.bg} rounded-2xl overflow-hidden relative`}>
          <div className="flex flex-col md:flex-row items-center p-8 md:p-10 min-h-[300px] md:min-h-[360px]">
            <div className="flex-1 z-10">
              ...
              <p className="text-gray-500 text-sm mb-6 max-w-xs">{slide.desc}</p>
              <button className="btn-primary">Shop Now →</button>
            </div>
            <div className="relative w-56 h-56 md:w-72 md:h-72 flex-shrink-0 mt-6 md:mt-0">
              <Image src={slide.image} alt={slide.title} fill className="object-contain drop-shadow-2xl" />
            </div>
          </div>
          {/* Dots */}
          ...
          {sideDeals.map((deal, i) => (
            <div key={i} className="bg-gray-50 rounded-2xl p-5 flex items-center gap-4 hover:shadow-md transition-shadow cursor-pointer">
              <div className="flex-1">
                <h3 className="font-bold text-dark text-base mb-1 leading-tight">{deal.title}</h3>
                <p className="text-xs text-gray-400 mb-2">{deal.tag}</p>
                <div className="flex items-center gap-2">
                  <span className="text-accent font-bold text-lg">{deal.price}</span>
                  <span className="text-gray-400 text-sm line-through">{deal.original}</span>
                </div>
              </div>
